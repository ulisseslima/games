...
public class PhaserEffect implements SampleProviderIntfc {
	 ...
	 * 
	 * @param bypassed
	 *            If true the effect is bypassed; if false the effect is
	 *            operational
	 */
	 ...
	 * 
	 * @param dryWetMixPercent
	 *            The percentage of wet signal in the mix
	 */
	public void setDryWetMixPercent(double dryWetMixPercent) {

		dryWetMixPercent = (dryWetMixPercent < PHASER_DRYWETMIXPERCENT_MIN) ? PHASER_DRYWETMIXPERCENT_MIN
				: dryWetMixPercent;
		dryWetMixPercent = (dryWetMixPercent > PHASER_DRYWETMIXPERCENT_MAX) ? PHASER_DRYWETMIXPERCENT_MAX
				: dryWetMixPercent;
	 ...
	 * <p>
	 * Sweep rate frequency must be between PHASER_SWEEPRATE_MIN_HZ and
	 * PHASER_SWEEPRATE_MAX_HZ
	 * 
	 * @param sweepRate
	 *            The rate of frequency sweep in Hz.
	 */
	public void setSweepRate(double sweepRate) {

		sweepRate = (sweepRate < PHASER_SWEEPRATE_MIN_HZ) ? PHASER_SWEEPRATE_MIN_HZ
				: sweepRate;
		sweepRate = (sweepRate > PHASER_SWEEPRATE_MAX_HZ) ? PHASER_SWEEPRATE_MAX_HZ
				: sweepRate;
	 ...
	 * <p>
	 * Sweep range must be between PHASER_SWEEPRANGE_MIN and
	 * PHASER_SWEEPRANGE_MAX
	 * 
	 * @param sweepRangeInOctaves
	 *            The range of frequency sweep in octaves.
	 */
	public void setSweepRangeInOctaves(int sweepRangeInOctaves) {

		sweepRangeInOctaves = (sweepRangeInOctaves < PHASER_SWEEPRANGE_MIN) ? PHASER_SWEEPRANGE_MIN
				: sweepRangeInOctaves;
		sweepRangeInOctaves = (sweepRangeInOctaves > PHASER_SWEEPRANGE_MAX) ? PHASER_SWEEPRANGE_MAX
				: sweepRangeInOctaves;
	...
	/**
	 * Sets the feedback level which is the amount of output feedback to the
	 * input.
	 * 
	 * @param feedbackPercent
	 *            The percentage of the output feedback to the input
	 */
	public void setFeedbackPercent(double feedbackPercent) {

		feedbackPercent = (feedbackPercent < PHASER_FEEDBACKPERCENT_MIN) ? PHASER_FEEDBACKPERCENT_MIN
				: feedbackPercent;
		feedbackPercent = (feedbackPercent > PHASER_FEEDBACKPERCENT_MAX) ? PHASER_FEEDBACKPERCENT_MAX
				: feedbackPercent;
	 ...
	 * 
	 * @param provider
	 *            The provider of samples for this PhaserEffect
	 */
	...
	private void initialize() {
		...
		wp = minWp = (2.0 * Math.PI * PHASER_BASE_FREQUENCY)
				/ SamplePlayer.SAMPLE_RATE;
		...
		currentStep = step = Math.pow(freqRange, sweepRate
				/ (SamplePlayer.SAMPLE_RATE / 2.0));
	}
